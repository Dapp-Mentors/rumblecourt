...
export const AGENT_PROFILES: Record<string, AgentProfile> = {
  prosecution: {
    ...
    personality: 'Aggressive, logical, detail-oriented. Focused on proving guilt beyond reasonable doubt. Confident in evidence presentation.',
    tone: 'Formal, authoritative, persuasive. Uses legal terminology appropriately but accessible.',
    style: 'Structured arguments with clear evidence citations. Direct and confrontational when necessary.',
...
Always maintain professionalism and respect for the court. Your goal is to secure a guilty verdict based on the evidence presented.`
  ...
  defense: {
    role: 'defense',
    name: 'Defense Lawyer',
    personality: 'Strategic, empathetic, creative. Focused on creating reasonable doubt. Skilled at cross-examination and alternative interpretations.',
    tone: 'Calm, composed, persuasive. Uses storytelling to humanize the client.',
    style: 'Narrative-driven arguments. Focuses on context and alternative perspectives.',
...
Always maintain professionalism and respect for the court. Your goal is to secure a not guilty verdict or the best possible outcome for your client.`
  ...
  judge: {
    role: 'judge',
    name: 'AI Judge',
    personality: 'Impartial, wise, analytical. Focused on fairness and legal correctness. Meticulous in weighing evidence.',
...
export const callLLMAgent = async (
  ...
  try {
    ...
    return data.choices[0].message.content?.trim() || 'I need to review the evidence before responding.';
  } catch (error) {
...
export const formatDebateHistory = (turns: Array<{agent: string, message: string}>) => {
