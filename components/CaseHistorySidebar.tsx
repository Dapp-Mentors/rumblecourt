          ...
          cases.map((case_) => (
            ...
            >
              ...
              <div className="mt-2 flex items-center justify-between">
                <span className={`text-xs px-2 py-0.5 rounded-full ${case_.status === 'active' ? 'bg-green-500/20 text-green-300' :
                  case_.status === 'completed' ? 'bg-blue-500/20 text-blue-300' :
                    'bg-slate-500/20 text-slate-300'
                  }`}>
                  {case_.status}
                </span>
                <span className="text-xs text-slate-600">
                  {case_.evidence.length} evidence
                </span>
              </div>
            </button>
          ))
