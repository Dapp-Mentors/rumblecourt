...
contract RumbleCourt {
    ...
    enum CaseStatus { PENDING, IN_TRIAL, COMPLETED, APPEALED }
    enum VerdictType { GUILTY, NOT_GUILTY, SETTLEMENT, DISMISSED }
    ...
    event CaseFiled(uint256 indexed caseId, address indexed plaintiff, string caseTitle);
    event TrialStarted(uint256 indexed caseId);
    event VerdictRecorded(uint256 indexed caseId, VerdictType verdictType, bool isFinal);
    ...
    /**
     * @dev Start trial for a pending case (owner/system only)
     * @param caseId The case identifier
     */
    function startTrial(uint256 caseId) external onlyOwner caseExists(caseId) {
        require(cases[caseId].status == CaseStatus.PENDING, "Case must be pending");
    ...
    function recordVerdict(
        ...
        require(cases[caseId].status == CaseStatus.IN_TRIAL, "Case must be in trial");
    ...
    function appealCase(uint256 caseId) external caseExists(caseId) {
        require(cases[caseId].plaintiff == msg.sender, "Only plaintiff can appeal");
        require(cases[caseId].status == CaseStatus.COMPLETED, "Case must be completed");
     ...
     * @return Case struct
     */
    function getCase(uint256 caseId) external view caseExists(caseId) returns (Case memory) {
        return cases[caseId];
    }
     ...
     */
    function getVerdict(uint256 caseId) external view caseExists(caseId) returns (Verdict memory) {
     ...
     */
    function hasVerdict(uint256 caseId) external view caseExists(caseId) returns (bool) {
